...
namespace Trackable.Web.Controllers
{
    ...
    public class DevicesController : ControllerBase
    {
            ...
            [FromQuery] string name = null)
        {
            ...
            {
                var results = await this.deviceService.FindByNameAsync(name);
                return this.dtoMapper.Map<IEnumerable<TrackingDeviceDto>>(results);
            }
            ...
            if (includesAllTags)
            {
                var results = await this.deviceService.FindContainingAllTagsAsync(tagsArray);
                return this.dtoMapper.Map<IEnumerable<TrackingDeviceDto>>(results);
            }
            else
            {
                var results = await this.deviceService.FindContainingAnyTagsAsync(tagsArray);
                return this.dtoMapper.Map<IEnumerable<TrackingDeviceDto>>(results);
            }
        }
